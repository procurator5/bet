...
//  Parallax Background Image
    ...
    // Get crypto currencies data for Marquee

    if( $(".ts-marquee").length ){
        getPrices();
        setInterval( getPrices, 60000);
    }

    // Calculate currency conversion on change of select element

    doExchange();
    $("#from_symbol_select, #to_symbol_select, #from_symbol_input").on("change", function () {
        doExchange();
    });
...
function doExchange(){
    var fromSymbolSelected = $("#from_symbol_select").find(":selected").text();
    var toSymbolSelected = $("#to_symbol_select").find(":selected").text();

    $.get("https://min-api.cryptocompare.com/data/price?fsym=" + fromSymbolSelected + "&tsyms=" + toSymbolSelected + "", function(data) {
        $("#to_symbol_input").val( $("#from_symbol_input").val() * data[toSymbolSelected] );
    });
}
            ...
            for( var e = 0; e < Object.keys(data["DISPLAY"][firstSymbol]).length; e++ ){
                ...
                if( firstSymbol !== secondSymbol ){
                    ...
                    if( marqueeInitialized === 0 ){
                        ...
                        setTimeout(function () {
                            $(".ts-crypto-prices").find("small, h4").removeClass("fade-in");
                        }, 300);
                        $(".ts-crypto-prices").find("[data-currency="+crossSymbols+"]").find("h4").replaceWith(
                            ( changePtc24Hour < 0 ? '<h4 class="my-3 text-danger ts-opacity-80">' + changePtc24Hour + '%</h4>': '<h4 class="my-3 text-success ts-opacity-80">' + changePtc24Hour + '%</h4>')
                        );
                    }
                }
